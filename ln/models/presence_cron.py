...
class PresenceCron(models.Model):
    ...
    def run_cron(self):
        ...
        today_start = datetime.datetime.now().replace(hour=0, minute=0, second=0) - datetime.timedelta(days=3)
        today_end = datetime.datetime.now().replace(hour=0, minute=0, second=3)
            ...
            if slot.employee_id and not slot.has_synced and slot.synchro_attendance:
                ...
                if slot.start_datetime.date() != slot.end_datetime.date():
                    attendance = self.env["hr.attendance"].create({
                        ...
                        "check_out": datetime.datetime(slot.start_datetime.year, slot.start_datetime.month, slot.start_datetime.day, 0, 0, 0) + datetime.timedelta(days=1) - datetime.timedelta(seconds=3600),
                    ...
                    while new_day.date() != slot.end_datetime.date():
                        attendance = self.env["hr.attendance"].create({
                            ...
                            "check_out": datetime.datetime(new_day.year, new_day.month, new_day.day, 0, 0, 0) + datetime.timedelta(days=1) - datetime.timedelta(seconds=3600),
                            "employee_id": slot.employee_id.id,
                            "shift_id": slot.id,
                        })
                        attendance.apply_rules()

...
class PresenceCron(models.Model):
    ...
    def run_cron(self):
        ...
        today_start = datetime.datetime.now(pytz.timezone('Europe/Paris')).replace(hour=0, minute=0, second=0) - datetime.timedelta(days=3)
        today_end = datetime.datetime.now(pytz.timezone('Europe/Paris')).replace(hour=0, minute=0, second=3)
